...
async function installCli(name: 'cursor' | 'codex' | 'claude') {
  ...
  try {
    ...
    message.info(`正在安装 ${name} CLI…`)
    await pollInstallLog()
    message.success(`${name} CLI 安装完成（或请终端验证）`)
  } catch (e) {
...
async function saveDevSettings() {
  ...
  try {
    ...
    if (codexKey && data.codexLogin && !data.codexLogin.ok) {
      message.warning(`Codex Key 已保存，但 CLI 登录未确认：${data.codexLogin.error || 'unknown'}`)
    } else {
      ...
      <NSpace vertical size="large" style="margin-top: 16px">
        ...
        <NForm label-placement="top" style="max-width: 560px">
          <NFormItem label="Cursor API key" :show-feedback="false">
            <NInput v-model:value="cursorApiKey" type="password" show-password-on="click" :placeholder="cursorApiKeySet ? '留空保持不变' : 'cursor.com/dashboard'" />
          </NFormItem>
          <NFormItem label="Claude API key" :show-feedback="false">
            <NInput v-model:value="claudeApiKey" type="password" show-password-on="click" :placeholder="claudeApiKeySet ? '留空保持不变' : 'console.anthropic.com'" />
          </NFormItem>
          <NFormItem label="Codex / OpenAI API key" :show-feedback="false">
            <NInput v-model:value="codexApiKey" type="password" show-password-on="click" :placeholder="codexApiKeySet ? '留空保持不变' : 'platform.openai.com/api-keys'" />
          </NFormItem>
