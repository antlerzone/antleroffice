...
export const useChannelManagementStore = defineStore('channel-management', () => {
  ...
  async function refreshAll(): Promise<void> {
    ...
    try {
      ...
      if (/gateway not connected/i.test(msg)) {
        lastError.value =
          'OpenClaw Gateway is not connected. Regional channels (QQ/Feishu) need the gateway — use Connect channel above for WhatsApp/Telegram.'
      } else if (/rpc call failed/i.test(msg)) {
        lastError.value =
          'Could not reach OpenClaw Gateway. Check that it is running and auth token matches ~/.openclaw/openclaw.json.'
      } else {
        lastError.value = msg
      }
      throw error
