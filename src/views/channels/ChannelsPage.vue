...
onMounted(() => {
  if (props.embedded) {
    // Delay until SSE gateway state may arrive; avoid noisy RPC error on first paint.
    setTimeout(() => {
      handleRefresh().catch(() => {})
    }, 800)
  } else {
...
<template>
  <NSpace vertical :size="16">
    <NCard :title="embedded ? undefined : t('pages.channels.title')" :bordered="!embedded" :class="{ 'channel-root-card': true, 'channel-embedded': embedded }">
