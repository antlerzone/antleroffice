...
import { resolveOpenClawGatewayAuth } from './openclaw-env.js'
...
function loadEnvConfig() {
  if (!existsSync(envPath)) {
    const auth = resolveOpenClawGatewayAuth({})
    return {
      PORT: 3001,
      OPENCLAW_WS_URL: 'ws://localhost:18789',
      OPENCLAW_AUTH_TOKEN: auth.OPENCLAW_AUTH_TOKEN,
      OPENCLAW_AUTH_PASSWORD: auth.OPENCLAW_AUTH_PASSWORD,
      OPENCLAW_TOKEN_SOURCE: auth.tokenSource,
  ...
  const auth = resolveOpenClawGatewayAuth(parsed)
  return {
...
if (envConfig.OPENCLAW_TOKEN_SOURCE === 'openclaw.json') {
  console.log('[Gateway] Using auth token from ~/.openclaw/openclaw.json')
}
...
app.post('/api/auth/login', (req, res) => {
  ...
  // Dev-friendly: accept any username/password while AUTH_* env vars are set.
  ...
  sessions.set(token, { username: user, expires })
...
app.post('/api/rpc', authMiddleware, async (req, res) => {
  ...
  if (!gateway.isConnected) {
    return res.status(503).json({
      ...
      error: {
        message:
          'Gateway not connected. Ensure OpenClaw is running and OPENCLAW_AUTH_TOKEN matches ~/.openclaw/openclaw.json.',
      },
    })
  }
...
const officePaPath = join(__dirname, '../public/office-pa')
if (existsSync(officePaPath)) {
